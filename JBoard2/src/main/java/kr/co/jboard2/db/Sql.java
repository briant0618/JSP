...
public class Sql {
	...
	public static final String SELECT_COUNT_ARTICLE = "SELECT COUNT(*) FROM `JBoard_Article` WHERE `parent`=0"; // all하면 댓글까지 계산되서 댓글빼고 계산 추가함
	...
	public static final String SELECT_ARTICLES = "SELECT a.*, b.`nick` FROM `JBoard_Article` AS a "  // Join
													...
													+ "On a.uid = b.uid "
													+ "WHERE `parent` = 0 " // 댓글도 계산되서 댓글 빼야함!
													+ "ORDER BY `seq` DESC "
													+ "LIMIT ?,10";
