	...
	jQuery(function(){
		...
		$('.btnCommentModify').click(function(e){
			...
			if(mode == '수정'){
				...
				$.ajax({
					url: '/Farmstory1/board/proc/commentModify.jsp',
					type: 'post',
					data: jsonData,
					dataType: 'json',
...
<section id="board" class="view">
    ...
    <table>
        ...
        <tr>
            ...
            <td>
                ...
                <span>[Download <%= article.getFb().getDownload() %>회]</span>
            </td>
        </tr>
        <% } %>
        <tr>
    ...
    <div>
        <% if(mb.getUid().equals(article.getUid())){ %>
        	<a href="/Farmstory1/board/proc/delete.jsp?group=<%= group %>&cate=<%= cate %>&seq=<%= article.getSeq() %>&parent=<%= article.getSeq() %>" class="btnDelete">삭제</a>
